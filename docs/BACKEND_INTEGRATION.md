...
**Two notification transports (both via the same FutoRide bot):**
- **Personal pings** — driver dispatch, driver cancel/expiry pings, and bus-proximity alerts — are sent **directly** through our own bot (native Telegram `sendMessage` to the user's `chatId`). They need the Connect-Telegram handshake above; no `chatId` = silently skipped.
- **Group safety alerts** — `/sos`, `/incidents/report`, and the stranded-student alert — go to the **SUG Security group via Alerta** (`ALERTA_TELEGRAM_TARGET` = the `TG_ALT_…` channelRef). This is a backend-only path (no per-user step); the app just calls the endpoint.
